...
*** -> Find the minimum difference of si and sj such that (i<j). 
In other words, find the minimum difference over all possible pairs of the n words.
...
int main() {
	...
	while(t--) {
		...
		for (int i=0;i<n; i++) {
			for (int j = i+1; j<n; j++) {
				ans = min(ans, cost(strVec[i], strVec[j]));
			}
		}
